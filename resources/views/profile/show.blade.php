    ...
    <div class="container">
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
            <div>
                ...
                <p class="text-muted mb-0">Data berikut diambil otomatis dari akun SIAP Kerja.</p>
            </div>
        ...
        <div class="mt-5">
            <div class="row g-4">
                                    ...
                                    <table class="table table-sm align-middle">
                                        ...
                                        <tbody>
                                            @foreach($user->educations as $edu)
                                                <tr>
                                                    <td>
                                                        <div class="fw-semibold">{{ $edu->school_name ?? '-' }}</div>
                                                        <div class="text-muted small">{{ $edu->study_field_name ?? '-' }}</div>
                                                    </td>
                                                    <td>{{ $edu->graduate_name ?? '-' }}</td>
                                                    <td>{{ $edu->start_year ?? '-' }} - {{ $edu->finish_year ?? '-' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                        ...
                                        <tbody>
                                            @foreach($user->experiences as $exp)
                                                <tr>
                                                    ...
                                                    <td>{{ $exp->job_title ?? '-' }}</td>
                                                    <td>{{ $exp->start_month ?? '-' }}/{{ $exp->start_year ?? '-' }} - {{ $exp->finish_month ?? '-' }}/{{ $exp->finish_year ?? '-' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                    ...
                                    <table class="table table-sm align-middle">
                                        ...
                                        <tbody>
                                            @foreach($user->trainings as $training)
                                                <tr>
                                                    <td>
                                                        <div class="fw-semibold">{{ $training->training_program_name ?? '-' }}</div>
                                                        <div class="text-muted small">{{ $training->training_center_name ?? '-' }}</div>
                                                    </td>
                                                    <td>{{ $training->vocational_name ?? '-' }}</td>
                                                    <td>{{ $training->start_month ?? '-' }}/{{ $training->start_year ?? '-' }} - {{ $training->finish_month ?? '-' }}/{{ $training->finish_year ?? '-' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                        ...
                                        <tbody>
                                            @foreach($user->certifications as $cert)
                                                <tr>
                                                    ...
                                                    <td>{{ $cert->issued_month ?? '-' }}/{{ $cert->issued_year ?? '-' }} - {{ $cert->expire_month ?? '-' }}/{{ $cert->expire_year ?? '-' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                ...
                <div class="col-lg-6">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body">
                            <h5 class="fw-bold mb-3">Keterampilan</h5>
                ...
                <div class="col-lg-6">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body">
                            <h5 class="fw-bold mb-3">Bahasa</h5>
                                        ...
                                        <tbody>
                                            @foreach($user->languages as $language)
                                                <tr>
                                                    ...
                                                    <td>{{ $language->proficiency_name ?? '-' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
